...
trust_colors <- c("#d73027", "#f46d43", "#fee090", "#74add1", "#2166ac")
...
race_trust <- df %>%
  filter(!is.na(trust_clean), !is.na(trust_numeric), !is.na(race_clean)) %>%
  mutate(
  ...
  group_by(race_short) %>%
  summarise(mean_trust = mean(trust_numeric, na.rm = TRUE),
            n = n(), .groups = "drop")
...
g2 <- ggplot(race_trust, aes(x = reorder(race_short, mean_trust),
  ...
  scale_y_continuous(limits = c(0, 5.2),
                     ...
                     labels = c("1\n(Never)", "2\n(Some)", "3\n(Half)", "4\n(Most)", "5\n(Always)")) +
  labs(
    title    = "Average Trust in Government by Race/Ethnicity",
    subtitle = "Higher score = more trust  |  Scale: 1 (Never) to 5 (Always)",
    x        = "Race / Ethnicity",
    y        = "Mean Trust Score"
  ) +
  my_theme
...
income_trust <- df %>%
  filter(!is.na(trust_clean), !is.na(trust_numeric), !is.na(income_category)) %>%
  mutate(
    trust_clean     = factor(trust_clean, levels = trust_levels),
...
g3 <- ggplot(income_trust, aes(x = income_category, y = mean_trust,
  ...
  scale_y_continuous(limits = c(0, 5.2),
                     ...
                     labels = c("1\n(Never)", "2\n(Some)", "3\n(Half)", "4\n(Most)", "5\n(Always)")) +
  labs(
    title    = "Average Trust in Government by Household Income",
    subtitle = "Higher score = more trust  |  Scale: 1 (Never) to 5 (Always)",
...
edu_trust <- df %>%
  filter(!is.na(trust_clean), !is.na(trust_numeric), !is.na(education_category)) %>%
  mutate(
    trust_clean        = factor(trust_clean, levels = trust_levels),
...
age_trust <- df %>%
  filter(!is.na(trust_clean), !is.na(trust_numeric), !is.na(age_clean)) %>%
  mutate(
    trust_clean = factor(trust_clean, levels = trust_levels),
...
g5 <- ggplot(age_trust, aes(x = age_group, y = mean_trust, fill = age_group)) +
  ...
  scale_y_continuous(limits = c(0, 5.2),
                     ...
                     labels = c("1\n(Never)", "2\n(Some)", "3\n(Half)", "4\n(Most)", "5\n(Always)")) +
  labs(
    title    = "Average Trust in Government by Age Group",
    subtitle = "Higher score = more trust  |  Scale: 1 (Never) to 5 (Always)",
    x        = "Age Group",
    y        = "Mean Trust Score"
  ) +
...
party_trust <- df %>%
  filter(!is.na(trust_clean), !is.na(trust_numeric), !is.na(party_clean)) %>%
  mutate(
    trust_clean = factor(trust_clean, levels = trust_levels),
...
g6 <- ggplot(party_trust, aes(x = party_short, y = mean_trust, fill = party_clean)) +
  ...
  scale_y_continuous(limits = c(0, 5.2),
                     ...
                     labels = c("1\n(Never)", "2\n(Some)", "3\n(Half)", "4\n(Most)", "5\n(Always)")) +
  labs(
    title    = "Average Trust in Government by Party Identification",
    subtitle = "Higher score = more trust  |  Scale: 1 (Never) to 5 (Always)",
